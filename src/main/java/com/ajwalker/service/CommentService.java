...
public class CommentService {
		...
		return comments.stream().map(comment -> {
			...
			PersonalDocument personalDocument = personalDocumentService.findById(comment.getUserId())
			                                                           .orElseThrow(() -> new RuntimeException(
																			   "Personal document not found for " +
			...
			Company company = companyService.findById(comment.getCompanyId())
			                                .orElseThrow(() -> new RuntimeException("Company not found for comment ID:" +
					                                                                        " " + comment.getId()));
			
			// DTO'ya veri ekliyoruz
			return new CommentCardResponseDto(company.getCompanyLogo(),  // Şirket logosu dinamik
			                                  ...
			                                  personalDocument.getPosition()
			                                                  .toString(),  // Pozisyon (Enum'dan string'e çeviriyoruz)
	...
	public Boolean addComment(AddCommentRequestDto dto, Long managerId) {
		...
		Comment comment = Comment.builder().userId(managerId).companyId(user.getCompanyId()).content(dto.content())
		                         .description(dto.description()).commentDate(LocalDate.now()).build();
